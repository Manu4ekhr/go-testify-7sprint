...
func TestMainHandlerWhenCountMoreThanTotal(t *testing.T) {
	...
	require.NotEmpty(t, responseBody, "Response body")

	cafeCount := len(strings.Split(responseBody, ","))
	assert.Equal(t, totalCount, cafeCount, "Должны вернуться 4 кафе")
}
...
func TestMainHandlerValidRequest(t *testing.T) {
	...
	assert.Equal(t, expected, responseRecorder.Body.String(), "Должны вернуться 2 первых кафе")
}
...
func TestMainHandlerCity(t *testing.T) {
	...
	assert.Equal(t, expected, responseRecorder.Body.String(), "Expected wrong city error")
}
